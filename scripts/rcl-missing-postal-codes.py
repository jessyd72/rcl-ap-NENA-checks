...
Data Quality Checks
Missing Postal Code - Road Centerline

This script will find road segments that 
are missing right and left postal codes. '''
...
def missingPostalCode(rcl):
    ...
    q = "({0} IS NULL OR {0} = '0') OR ({1} IS NULL OR {1} = '0')".format(lzip, rzip)
    select = arcpy.SelectLayerByAttribute_management(rcl, 'NEW_SELECTION', q)

    if arcpy.GetCount_management(select)[0] != '0':

        out = arcpy.CopyFeatures_management(select, out_fgdb + r'\checkRCL_Missing_Postal_Code')

        keep_flds = ['OBJECTID', 'RCL_ID_GeoAdd', lzip, rzip]
        ...
        print_to_stdout('No missing postal codes')
        return('')
...
if __name__ == '__main__':
    ...
    try:
        ...
        logfile = (path.abspath(path.join(path.dirname(__file__), '..', r'logs\Missing_Postal_Code_log_{0}_{1}.txt'.format(current.month, current.year))))
        ...
        print_to_stdout('Finding missing postal codes...')
        out_check = missingPostalCode(input_rcl)
        if out_check != '':
