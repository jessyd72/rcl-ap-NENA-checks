...
Data Quality Checks
Multipart Segments - Road Centerline

This script finds multipart road segments.'''
...
def multipartSegments(rcl):
    ...
    with arcpy.da.SearchCursor(rcl, ['RCL_ID_GeoAdd', 'shape@']) as scur:
        for row in scur:
            if row[1].isMultipart:
                problem_children.append(str(row[0]))

    if problem_children:

        q = 'RCL_ID_GeoAdd IN ({})'.format(','.join(problem_children))
        select = arcpy.SelectLayerByAttribute_management(rcl, 'NEW_SELECTION', q)

        out = arcpy.CopyFeatures_management(select, out_fgdb + r'\checkRCL_Multipart_Segments')
        ...
        print_to_stdout('No multipart roads found')
        return('')
...
if __name__ == '__main__':
    ...
    try:
        ...
        logfile = (path.abspath(path.join(path.dirname(__file__), '..', r'logs\Multipart_log_{0}_{1}.txt'.format(current.month, current.year))))
        ...
        out_check = multipartSegments(input_rcl)
        if out_check != '':
