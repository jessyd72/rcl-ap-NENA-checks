...
Data Quality Checks
Missing Postal Code - AP

This script will find address points that 
...
def apMissingPostalCode(ap):
    ...
    q = "{0} IS NULL OR {0} = '0'".format(ap_zip)
    select = arcpy.SelectLayerByAttribute_management(ap, 'NEW_SELECTION', q)

    if arcpy.GetCount_management(select)[0] != '0':
        
        out = arcpy.CopyFeatures_management(select, out_fgdb + r'\check_AP_Missing_Postal_Code')
...
if __name__ == '__main__':
    ...
    try:
        ...
        logfile = (path.abspath(path.join(path.dirname(__file__), '..', r'logs\AP_Missing_Postal_Code_log_{0}_{1}.txt'.format(current.month, current.year))))
        ...
        out_check = apMissingPostalCode(input_ap)
        if out_check != '':
